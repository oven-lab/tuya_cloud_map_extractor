...
def download_map(url: dict) -> dict:
    ...
    _LOGGER.debug("Response: " + response.status_code, base64.b64encode(response.content), base64.b64encode(bytes(str(url), 'utf-8')))
    try:
        data = response.json()
        ...
        _LOGGER.error('Unsupported data type. Include the following data in a github issue to request the data format to be added: ', response.status_code, base64.b64encode(response.content), base64.b64encode(bytes(str(url), 'utf-8')), ' Thank you!')
